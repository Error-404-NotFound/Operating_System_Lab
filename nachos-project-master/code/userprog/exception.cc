...
void handle_SC_Halt() {
    ...
    kernel->currentThread->endTime = clock();
    cout << "Time: " << (double)(kernel->currentThread->endTime - kernel->currentThread->startTime)/CLOCKS_PER_SEC << "s\n";
    SysHalt();
    ASSERTNOTREACHED();
}
...
void handle_SC_Sub() {
    DEBUG(dbgSys, "Sub " << kernel->machine->ReadRegister(4) << " - "
                         << kernel->machine->ReadRegister(5) << "\n");
...
void handle_SC_Div() {
    DEBUG(dbgSys, "Div " << kernel->machine->ReadRegister(4) << " / "
                         << kernel->machine->ReadRegister(5) << "\n");
...
void handle_SC_OSSleep() {
    int seconds = kernel->machine->ReadRegister(4);
    SysSleep(seconds*250000);
    return move_program_counter();
}
    ...
    switch (which) {
        ...
        case SyscallException:
            switch (type) {
		        ...
		        case SC_Div:
		            return handle_SC_Div();
                case SC_OSSleep:
                    return handle_SC_OSSleep();
		        case SC_ReadNum:
